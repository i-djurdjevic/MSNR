...
\author{Isidora Đurđević, Ana Stanković, Milica Đurić\\ kontakt email prvog, anastankovic167@gmail.com (trećeg) autora}
\date{5.~april 2017.}
\maketitle
...
Opišimo sada značenje za obe vrste operacione semantike, prirodne i strukturno operacione, \textit{tranzicionim sistemom}. Ona će imati dve vrste konfiguracija \cite{wiley}:
\begin{itemize}
...
\begin{center}$\langle$ \textit{$S_{1}$, $s_{1}$} $\rangle$ $\rightarrow$ \textit{$s'_{1}$}, ..., $\langle$ \textit{$S_{n}$, $s_{n}$} $\rangle$ $\rightarrow$ \textit{$s'_{n}$} \end{center}
% fali deo formule
