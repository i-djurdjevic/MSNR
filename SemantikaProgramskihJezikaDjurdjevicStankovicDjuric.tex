...
Posmatrajmo tabelu ~\ref{tab:b}. Aksioma [$ass_{ns}$] govori da u stanju \textit{s}, naredba \textit{x := a} će dovesti do završnog stanja $s[x \mapsto A[[a]]s]$ što predstavlja isto stanje kao \textit{s}, samo što \textit{x} ima vrednost \textit{A[[a]]s]}. Objasnimo ovo na primeru, gde $s_0$ je stanje koje postavlja svim promenljivima vrednost na 0:  \begin{center} $\langle$ \textit{x := x + 1, $s_0$} $\rangle$ $\rightarrow$ \textit{$s_0$[x $\rightarrow$ 1]}
\end{center}
